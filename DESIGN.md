...
### Tooltip (Panel de reglas)
- **Style:** a `15px` inline-SVG info-circle icon (same `24×24`/`1.75`-stroke language as every other icon in the system) sits right after each control's label, muted `--text-muted` by default, `--blue-strong` on hover/focus. On hover or keyboard focus it reveals a `--text`-filled, white-text bubble (`12px/500`, `10px` radius, `--shadow-pop`) with a one-sentence plain-language explanation of that rule.
- **Pattern:** CSS-only disclosure (`:hover`/`:focus` on the trigger, `role="tooltip"` + `aria-describedby` linking trigger to bubble) — no JS is needed to show/hide it. A small global `keydown` listener blurs a focused `.tooltip-trigger` on Escape, satisfying dismissibility; the bubble living inside the same hoverable wrapper as the trigger satisfies the "hoverable" requirement for free.
- **Mobile (under 640px):** the bubble stops anchoring to the tiny icon and instead spans the full control row (`.tooltip` becomes non-positioning, `.control` becomes the positioning context, bubble gets `left/right: 12px` insets) — this is what keeps a long label's explanation from being pushed off the edge of a narrow viewport.
